...
export default function BatcherButtonTemplate(props) {
    ...
    return (
        <>
            <ImageModal imageRoute={
                    props.type === 'users' ? usersTemplate : props.type === 'sessions' ? sessionsTemplate : eventsTemplate
                }
                show={csvTemplateActive}
                close={closeCSVTemplate}/>
            <div className="csvLoaderButton">
                <CSVReader onFileLoaded={
                    (data, fileInfo, originalFile) => beforeLoad(data)
                }/>

            </div>
            <button className='csvTemplate'
                onClick={showCSVTemplate}>
