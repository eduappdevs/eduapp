...
import { getExtraFields , pushExtraFields ,updateExtraFields , deleteExtraFields} from '../services/extrafields.service';
...
export default function ExtraFields(props) {
    ...
    const UPDATE_EXTRAFIELD = (body)=>{
        ...
        updateExtraFields({
            table,
            id,
        },ex).then(()=>GET_EXTRAFIELDS)
    }

    const PUSH_EXTRAFIELD = (body)=>{
        pushExtraFields({table,id},{
            "name": body.name,
            "type": body.type,
            "value": ''
    ...
    return (
        ...
        onNoAction={() => {
            ...
            <StandardModal customOkay={'Close'} text={'Custom fields'} showLoader={loading} show={show && !deleteModal} hasTransition hasIconAnimation icon='success' onCloseAction={()=>{setShow(false)}}
                form={<><ul className='list'> {
                        extraFields && extraFields.map((field, index) => {
                            return (
                                <li key={index}>
                                    ...
                                    <input type={field.type} id={`input_field_${field.name}`} name={field.name} defaultValue={field.value} onChange={e=>
                                                UPDATE_EXTRAFIELD({name: field.name, value: e.target.value})
                                            }/>
                                    </div>
                                    <span className='delete-span' onClick={()=>{setDeleteModal(field.name)}}>
                        ...
                        <li className='new_extrafield'>
                            ...
                            <input type="text" placeholder='Name' id={`new_fieldName_${props.id}`}/>
                        ...
                        <button onClick={()=>{
                            let name = document.getElementById(`new_fieldName_${props.id}`).value
                            let type = document.getElementById(`new_fieldType_${props.id}`).value
                            name != '' && type != '' && PUSH_EXTRAFIELD({
                                name: name,
                                type: type,
                            })}}>+</button>
                        </li>
