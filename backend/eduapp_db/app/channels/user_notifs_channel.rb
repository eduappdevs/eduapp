  ...
  def subscribed
	...
	def receive(cmd)
		...
		when 'new_msgs'
			puts "hi"
		when 'keys_request'
			chat = ChatParticipant.where(chat_base_id: params[:chat_id])
			owner = nil
			counterpart = nil
			if chat.count > 0
				chat.each do |u|
					if u.user_id === @notifs_user
						owner = u
					else
						counterpart = u
					end
			...
			if !owner.nil? && !counterpart.nil?
				salt = Base64.encode64(ENV.fetch("ENCRYPTION_PATTERN"))[0..-2] 
				keys = { 
					"command" => "chat_keys", 
					"keys" => {
						owner_key: "#{salt}h#{Base64.encode64(owner.private_key)}", 
						counterpart_key: "#{salt}h#{Base64.encode64(counterpart.public_key)}", 
					} 
				}
				ActionCable.server.broadcast @chat_name, keys
